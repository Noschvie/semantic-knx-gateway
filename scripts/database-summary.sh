...
BASE_URL="${API_BASE_URL}/api/v2/database"
...
TOTAL_JOBS=$(echo "$CLEANUP_JOBS" | jq '.meta.pagination.total // 0')
LAST_JOB_TIME=$(echo "$CLEANUP_JOBS" | jq -r '.data[0].attributes.completed_at_iso // "N/A"')

# Get health checks
HEALTH_CHECKS=$(curl -s -X GET "${API_BASE_URL}/api/v2/stats/health/db-checks" \
  -H "Authorization: Bearer $TOKEN")
...
TOTAL_JOBS=$([ "$TOTAL_JOBS" = "null" ] && echo "0" || echo "$TOTAL_JOBS")
...
# Check if TABLES_JSON is valid JSON object (not null, not empty)
if echo "$TABLES_JSON" | jq -e 'type == "object"' &>/dev/null; then
  ...
  echo -e "${YELLOW}⚪ No table data available${NC}"
fi
...
if [ "$COVERAGE_DAYS" -gt 0 ]; then
  DAILY_GROWTH_KB=$(awk "BEGIN {printf \"%.1f\", $DB_SIZE_BYTES / $COVERAGE_DAYS / 1024}")
  YEARLY_PROJECTION_MB=$(awk "BEGIN {printf \"%.1f\", $DAILY_GROWTH_KB * 365 / 1024}")
else
  DAILY_GROWTH_KB="0"
