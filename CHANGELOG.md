...
### Added
- **Database Integrity Health Checks & Monitoring** — New comprehensive health check system:
  
  **REST API Endpoints:**
  - `GET /api/v2/stats/health/db-checks` — Complete integrity overview with 3 checks:
    - Orphaned States Check: States without corresponding datapoint mapping
    - Duplicate GA Check: Group addresses with multiple mappings (data corruption risk)
    - Stale Mappings Check: Mappings without a current state (unused entries)
  - `GET /api/v2/stats/health/orphaned-states?limit=50` — Detailed orphaned state listing
  - `GET /api/v2/stats/health/duplicate-gas` — Detailed duplicate GA investigation
  - `GET /api/v2/stats/health/stale-mappings?limit=50` — Detailed stale mapping listing
  
  **Periodic Statistics Logger Enhancement:**
  - `StatisticsLogger` now includes data integrity checks in periodic output
  - New output section: "🔍 DATA INTEGRITY CHECKS" with all three checks and Data Integrity Score
  - Calculates data integrity score: `(total_mappings - stale) / total_mappings * 100%`
  - Runs every 15 minutes alongside existing statistics
  
  **Specialized Maintenance Scripts:**
  - `scripts/database-summary.sh` — Refactored as monitoring dashboard:
    - 7 sections with complete database overview (system, events, tables, subscriptions, growth, integrity, maintenance)
    - Ideal for cron jobs and admin reports
    - REST API based on consistency
    - Enhanced header documentation
  - `scripts/db-health-check.sh` — Refactored as diagnostic & cleanup tool:
    - Specialized for troubleshooting and remediation
    - Flags: `--cleanup`, `--list-orphaned`, `--list-duplicates`, `--list-stale`
    - REST API-based (no Docker dependency required)
    - Intelligent status determination with recommendations
  
  **Documentation:**
  - `docs/DATABASE-MAINTENANCE-GUIDE.md` — Comprehensive maintenance guide:
    - Decision matrix for script selection
    - Detailed explanation of all three integrity checks
    - Recommended maintenance schedule (daily, weekly, monthly)
    - Cron job examples
    - REST API integration guide
    - Critical issues handling (duplicate GA protocol)
